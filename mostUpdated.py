...
def redrawAll(canvas, data):
    if data.screenMode==0:
        ...
        drawMainButton(canvas, 210, "EXPORT NOTES")
        #+30 = 240
        ...
        drawMainButton(canvas, 480, "ADD")
        drawMainButton(canvas, 540, "UPLOAD FILE")
        
        if data.mousePosition == "mode":
            drawLargeButton(canvas, 270, "MODE")
        elif data.mousePosition == "exportNotes":
            drawLargeButton(canvas, 210, "EXPORT NOTES")
...
def motion(event, data):
    ...
    if 30<=data.mouseX<=170 and 210-30<=data.mouseY<=210:
        data.mousePosition = "exportNotes"
